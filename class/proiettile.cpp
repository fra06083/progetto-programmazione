   #include "proiettile.hpp"
    ...
    bool Proiettile::isAttivo(){
        if (this == nullptr) {
        // Il puntatore this non è valido
        return false;
    }
    return sparato && x < 80;  // Aggiungi screenWidth come membro della classe
}

    void Proiettile::move(WINDOW *game, lista_proiettili *proiettile) {
            if(proiettile!=NULL){mvwaddch(game, y, x, ' ');
            wrefresh(game);
           // napms(50);
            x=x+1;
            mvwaddch(game, y, x, '*');
