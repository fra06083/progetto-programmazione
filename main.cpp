...
{
    // dichiarazioni puntatori che verranno passati alla claasse GAME
    Layout *layout = new Layout();
    Map *map = new Map();
    Player *player = new Player(MAX_X-START_X+1, MAX_Y-2, 5, map, 40); // Impostazione iniziale del personaggio.
    p_base_en base_enemy = NULL;
   
    p_pro proiettile=NULL;
    Game *game = new Game(layout, map, player, base_enemy, proiettile);

    game->run(); // LOOP per non far chiudere il gioco
